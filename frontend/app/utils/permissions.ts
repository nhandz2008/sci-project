...
/**
 * Check if a user can edit a specific competition in creators mode
 * @param user - The current user
 * @param competition - The competition to check permissions for
 * @param isCreatorsMode - Whether the user is in creators mode
 * @returns true if the user can edit the competition, false otherwise
 */
export const canEditCompetitionInCreatorsMode = (user: User | null, competition: Competition, isCreatorsMode: boolean): boolean => {
  if (!user) {
    return false;
  }

  // If not in creators mode, use normal permission logic
  if (!isCreatorsMode) {
    return canEditCompetition(user, competition);
  }
