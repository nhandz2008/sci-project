...
class TestCompleteUserFlow:
    ...
    def test_complete_user_registration_flow(
        ...
        new_login_data = {"email": "flow@example.com", "password": "NewFlowPass123"}
    ...
    def test_complete_admin_flow(self, client: TestClient, session: Session):
        ...
        # 2. Login as admin
        login_data = {"email": "admin_flow@example.com", "password": "AdminFlowPass123"}
        login_response = client.post("/api/v1/auth/login", json=login_data)
        assert login_response.status_code == status.HTTP_200_OK
        ...
        deactivate_response = client.put(
            f"/api/v1/users/{regular_user.id}/deactivate", headers=headers
        )
        assert deactivate_response.status_code == status.HTTP_200_OK

        # 7. Activate regular user
        activate_response = client.put(
            f"/api/v1/users/{regular_user.id}/activate", headers=headers
        )
        assert activate_response.status_code == status.HTTP_200_OK

        # 8. Change user role to admin
        role_response = client.put(
            f"/api/v1/users/{regular_user.id}/role?new_role=ADMIN", headers=headers
        )
    ...
    def test_complete_pagination_and_filtering_flow(
        ...
        pagination_response = client.get(
            "/api/v1/users?skip=0&limit=3", headers=headers
        )
        ...
        active_filter_response = client.get(
            "/api/v1/users?is_active=true", headers=headers
        )
        ...
        assert all(
            "Pagination" in user["full_name"] or "Pagination" in user["organization"]
    ...
    def test_complete_security_flow(self, client: TestClient, session: Session):
        ...
        expired_token = create_access_token(
            "some-user-id", expires_delta=timedelta(seconds=1)
        )
